...
(define (first-branch st) (stream-first st))
...
(define (get-ch-branch st ch)
  (if (st-empty? st) #f
      ; daca st nu e gol, atunci verifica daca primul caracter din eticheta primei
      ; ramuri a st este egal cu caracterul cerut si returneaza prima ramura,
      ; altfel apeleaza recursiv pe stiva functia pentru restul ramurilor
      (if (equal? (first (get-branch-label (first-branch st))) ch) (first-branch st)
